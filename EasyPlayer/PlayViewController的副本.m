...
static const char nalu_head_[4] = { 0x00,0x00,0x00,0x01 };
static const char nalu_head_ext[3] = { 0x00,0x00,0x01 };

int FindNaluHead(const unsigned char *data, const uint32_t datalen, int* splitSize)
{
	for (int i = 0; i <= datalen - 4; i++)
	{
		if (memcmp(data + i, nalu_head_, 4) == 0)
		{
			if(splitSize)
				*splitSize = 4;
			return i + 4;
		}
		else if (memcmp(data + i, nalu_head_ext, 3) == 0)
		{
			if(splitSize)
				*splitSize = 3;
			return i + 3;
...
//    return;
	...
	int splitSize = 0;
	int offset = FindNaluHead(frame, frameSize, &splitSize);
	idx += offset;
	int nalu_type = (frame[idx] & 0x1F);
    ...
    if (nalu_type == 7) {
		offset = FindNaluHead(frame + idx, frameSize - idx, &splitSize);
		if(offset > 0){
			_spsSize = offset - splitSize;
		}
		else{
			_spsSize = frameSize - idx;
		}
		
		if(_spsSize > 0){
			sps = malloc(_spsSize);
			if(sps)
				memcpy (sps, frame + idx, _spsSize);
		}
		
        if(offset > 0){
			idx += offset;
			nalu_type = (frame[idx] & 0x1F);
		}
    }
    ...
    if(nalu_type == 8) {
		offset = FindNaluHead(frame + idx, frameSize - idx, &splitSize);
		if(offset > 0){
			_ppsSize = offset - splitSize;
		}
		else{
			_ppsSize = frameSize - idx;
		}
		
		if(_ppsSize > 0){
			pps = malloc(_ppsSize);
			if(pps)
				memcpy (pps, frame + idx, _ppsSize);
		}
    ...
    if(nalu_type == 6) {//sei;
		...
		offset = FindNaluHead(frame + idx, frameSize - idx, &splitSize);
		if(offset > 0){
			seiSize = offset - splitSize;
		}
		else{
			seiSize = frameSize - idx;
		}
    ...
    if(nalu_type == 5) {
        while(idx < frameSize - 4){
            offset = FindNaluHead(frame + idx, frameSize - idx, &splitSize);
            if(offset > 0){
                blockLength = offset - splitSize;
            }
            else{
                blockLength = frameSize - idx;
            }
            //blockLength = frameSize - idx;
            data = malloc(blockLength + 4);
            memcpy(data+4, &frame[idx], blockLength);
            
            uint32_t dataLength32 = htonl (blockLength);
            memcpy (data, &dataLength32, sizeof (uint32_t));
            status = CMBlockBufferCreateWithMemoryBlock(NULL, data,  // memoryBlock to hold buffered data
                                                        blockLength + 4,  // block length of the mem block in bytes.
                                                        kCFAllocatorNull, NULL,
                                                        0, // offsetToData
                                                        blockLength + 4,   // dataLength of relevant bytes, starting at offsetToData
                                                        0, &blockBuffer);
    ...
    if (nalu_type == 1) {
        ...
        status = CMBlockBufferCreateWithMemoryBlock(NULL, data,  // memoryBlock to hold data. If NULL, block will be alloc when needed
                                                    blockLength+4,  // overall length of the mem block in bytes
                                                    kCFAllocatorNull, NULL,
                                                    0,     // offsetToData
                                                    blockLength+4,  // dataLength of relevant data bytes, starting at offsetToData
                                                    0, &blockBuffer);
        if(status != kCMBlockBufferNoErr){
            NSLog(@"CMBlockBufferCreateWithMemoryBlock ERROR type : %d",(int)status);
        }
