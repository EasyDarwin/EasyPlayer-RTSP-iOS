...
unsigned int DecodeVideo(void *DecHandle, DEC_DECODE_PARAM *pDecodeParam, DVDVideoPicture *picture)
{
    ...
    if (got_picture)
    {
        ...
        if (s_uiDecodeMethod == IDM_SW)
        {
            ...
            sws_scale (pComponent->pImgConvertCtx, pComponent->pFrame->data, 
                       pComponent->pFrame->linesize, 0, pComponent->pCodecCtx->height,
                       pComponent->picture.data, pComponent->picture.linesize);
